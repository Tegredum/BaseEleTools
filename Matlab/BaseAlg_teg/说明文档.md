...
## calc_par_drq_par_rq
计算矩阵 $\frac{\partial \left[\dot{r}; \dot{q}_{\rm{e}}; \dot{q}_{\rm{b}}\right]}{\partial \left[r; q_{\rm{e}}; q_{\rm{b}}\right]}$ 的方法。
根据 calc_drq_by_rqAndVel 给出的 $\left[\dot{r}; \dot{q}_{\rm{e}}; \dot{q}_{\rm{b}}\right]$ 的表达式，有：
$$\frac{{\partial \left[ {\dot r;{{\dot q}_{\rm{e}}};{{\dot q}_{\rm{b}}}} \right]}}{{\partial \left[ {r;{q_{\rm{e}}};{q_{\rm{b}}}} \right]}} = \left[ { \cdots ,\frac{\partial }{{\partial {{\left[ {r;{q_{\rm{e}}};{q_{\rm{b}}}} \right]}_{\left[ i \right]}}}}{{\left( {\frac{{\partial {\bf{E2V}}\left( {\left[ {r;{q_{\rm{e}}};{q_{\rm{b}}}} \right]} \right)}}{{\partial \left[ {r;{q_{\rm{e}}};{q_{\rm{b}}}} \right]}}} \right)}^{ - 1}}\left( {{{\bm{v}}_{\rm{t}}} - {{\bm{v}}_{\rm{m}}}} \right), \cdots } \right]$$
偏导数计算可能会出现奇异的情况。继承 TransUtils.par_vec_par_rtp_inv 的异常处理措施，在计算多个二阶偏导矩阵时，异常返回状态优先级别为：'zeroR' > 'vertical' > 'ok'。
## calc_par_drq_par_relVel
计算矩阵 $\frac{\partial \left[\dot{r}; \dot{q}_{\rm{e}}; \dot{q}_{\rm{b}}\right]}{\partial \left({\bm{v}}_{\rm{t}} - {\bm{v}}_{\rm{m}}\right)}$ 的方法。
根据 calc_drq_by_rqAndVel 给出的 $\left[\dot{r}; \dot{q}_{\rm{e}}; \dot{q}_{\rm{b}}\right]$ 的表达式，本方法只需要去调用 TransUtils.par_vec_par_rtp_inv 即可。
## calc_par_drh_par_qb
根据 calc_drh_by_qbAndVel 中给出的 ${\dot r}_{\rm{h}}$ 表达式，有：
$$\frac{{\partial {{\dot r}_{\rm{h}}}}}{{\partial {q_{\rm{b}}}}} =  - \left[ {\sin {q_{\rm{b}}},0,\cos {q_{\rm{b}}}} \right]\left( {{{\bm{v}}_{\rm{t}}} - {{\bm{v}}_{\rm{m}}}} \right)$$